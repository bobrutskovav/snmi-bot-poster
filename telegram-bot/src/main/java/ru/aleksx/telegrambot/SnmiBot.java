...
import ru.aleksx.telegrambot.props.BotProperties;
...
public class SnmiBot extends TelegramLongPollingBot {
    ...
    public boolean sendHtmlMessages(List<String> messages) {
        try {
            log.debug("Send Messages {}", messages);
            messages.forEach(this::sendHtmlMessage);
        } catch (TelegramApiException e) {
    ...
    private void sendHtmlMessage(String text) throws TelegramApiException {
        var msg = new SendMessage(botProperties.getChannelUuid(), text);
        msg.enableHtml(true);
        execute(msg);
    }
}
